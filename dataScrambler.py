...
class DataCreator():
    ...
    def createData(self, preguntas, comentarios):
        ...
        for i in range(1, preguntas):
            if (i % 468) == 0 and i >= 0:
                fecha += timedelta(1)
            pregunta = {"preg_id": i, "titulo": "Titulo {}".format(i), "texto": fake.text(), "date": str(fecha), "tema": random.choice(temas), "userid": randint(1, 200), "likes": randint(0, 100), "estatus": random.choice(estatus), "comments": []}
            for j in range(1, randint(1, comentarios)):
